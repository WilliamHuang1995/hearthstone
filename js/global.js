...
// Get current time for "lazyloading" (it's not really, we're just fading it in, and there's no scrolling involved)
...
// Fade the background in if it takes a while to load, and display instantly if it's fast/cached
...
// Because :active on Android is a big issue (?????) #AngryNotAngry
var ua = navigator.userAgent.toLowerCase();
var isAndroid = ua.indexOf('android') > -1;
if(isAndroid) {
	$('.push').removeClass('push').bind('touchstart', function(){
		$(this).addClass('press');
...
// Form submission without new page
$(function() {
	var inputMessage = $('.text'), // suggestion
			sendButton = $('.submit'); // submit button

	$('.submit').on('click', function(e) {
		e.preventDefault(); // prevent the form to do the post

		$(sendButton).val('Sending..').addClass('press');

		var xhr = new XMLHttpRequest();
		xhr.open('POST', '//formspree.io/hearthstone.party@gmail.com', true);
		xhr.setRequestHeader("Accept", "application/json")
		xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded")

		xhr.send(
			"Suggestion=" + $(inputMessage).val());

		xhr.onloadend = function (res) {
			if (res.target.status === 200){
				$(sendButton).val('All good!').prop('disabled', true).removeClass('press');
				$(inputMessage).blur().val('Thank you!').prop('disabled', true).css({'padding' : '16px 0', 'border-top' : '1px solid rgba(255,255,255,0.15)'});
			}
			else {
